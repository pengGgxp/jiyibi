...
- 本文边界：定义产品语义、计算口径、交互规则和验收标准；不代表功能已经实现
...
这是一份产品提案，不是已批准的实现计划。现有 MVP 约定“分析结果只在客户端派生，且不改变备份或同步协议”；因此本提案分为两层：

- **可在现有边界内讨论和验证的改进**：现金流与日常花法的文案分离，以及用测试账本验证现有算法。轻量确认不能只做成临时界面，否则刷新后会失效。
- **需要单独批准的领域扩展**：保存后轻量确认、待确认提醒、处理方式和恢复分摊都必须持久化，并同步到备份与云端。若不批准这项扩展，就不能宣称异常交易处理或多端分析口径一致；实现应继续使用当前“所有负金额纳入日常花法”的 MVP 规则。

本文后续的数据模型和协议段落描述的是第二层候选方案，不能在未批准前直接落地。
...
- 有效账目的有符号金额全部进入当前余额，见 [`calculateLedgerSummary`](../src/domain/stats.ts#L252)。
- 正金额不会进入消费速度，负金额会按绝对值进入支出统计，见 [`prepareExpenseData`](../src/domain/stats.ts#L107)。
- 所有有效负金额目前具有相同的分析语义，没有“一次性”“可报销”或“转账”状态。
- 统计窗口的最早日期可能由收入记录启动，因为代码先更新最早记录日，再跳过正金额，见 [`prepareExpenseData`](../src/domain/stats.ts#L121)。
- 最近最多 30 个已完成自然日按线性比例外推；14 天以下不输出预测结论，见 [`calculateSpendingAnalysis`](../src/domain/stats.ts#L341)。
- 分析结果本身只在本机派生，不持久化也不上传，见 [`SpendingAnalysis`](../src/domain/types.ts#L151)。
...
| 收入预期 | 下一次发薪日的最低收入与预计收入 | 目标日期、最低金额、预计金额 | 只用于下周期场景比较，不提前进入余额 |
...
```text
预计剩余日常支出 = 日常支出总额 × 距发薪日天数 ÷ 数据覆盖天数
预计周期末余额 = 当前余额 - 预计剩余日常支出
距周期底线 = 预计周期末余额 - 周期底线
每日可花 = max(当前余额 - 周期底线, 0) ÷ 距发薪日天数
```
...
- 新备份恢复后必须保留处理方式、确认状态与恢复分摊。
- 恢复预检必须验证分摊引用、方向、处理方式和累计金额；任一失败不得修改现有数据。
- 预览应显示账目数，不需要暴露分析派生结果。

### 9.3 云同步

- 当前同步协议 v4 无法表达新的账目语义，需要下一协议版本。
- 新协议可以继续接收 v1–v4 的默认语义账目；旧客户端缺少新字段时不得静默清除云端已有处理方式。
- 一旦账号中存在账户转账、一次性支出、可报销支出、退款/报销、待确认状态或恢复分摊，v1–v4 客户端不得继续执行云端拉取、推送或给出“同步完成”。服务端应返回明确的 `upgrade_required`，旧客户端保留本机数据并提示更新应用。
- 旧客户端离线期间产生的 outbox 必须在升级后由新客户端迁移再发送，不能由旧协议覆盖新语义；升级失败时本机账目不得丢失。
- 只有全部账目仍为默认普通收支且不存在恢复分摊时，服务端才可以向旧协议投影现有账本。
- 新客户端读取旧账目时使用保守默认，不伪造退款或转账关系。
- 关联的两笔账目及关联金额需要作为可冲突、可恢复的关系处理；不能依赖客户端时间戳静默覆盖。
- D1 需要保存处理方式、确认状态和恢复分摊，并校验两端属于同一用户代次、关联金额为正且不超过尚未恢复的支出金额。
...
| 事项 | 当前判断 | 风险 | 后续决定 |
|---|---|---|---|
| 异常交易触发阈值 | 待校准 | 过低会打扰，过高会遗漏 | 使用代表性测试账本比较触发率和结论变化 |
...
| 处理方式与恢复分摊持久化并同步 | 仅临时保存在界面或单设备 | 轻量确认若不持久化就无法在刷新、备份和多端后生效 | 需单独批准的范围扩展 | 云同步范围被取消 |
| 非默认语义要求旧客户端升级 | 继续向旧客户端投影所有账目 | 旧客户端无法计算正确余额和预测，兼容投影会造成假一致 | 本规范建议 | 旧协议获得等价语义支持 |

## 14. 推荐实施顺序

1. 修正统计窗口，使收入记录不能启动支出数据覆盖。
2. 增加账目处理方式与保守迁移，先完成本机余额和分析计算。
3. 实现异常交易保存后的轻量确认与待确认恢复入口。
4. 将实际现金流和日常花法在分析页中明确分开。
5. 升级加密备份、同步协议和 D1，并验证旧客户端保留新字段。
6. 使用覆盖全部场景的测试账本校准异常触发条件。
7. 更新 `README.md`、`PRIVACY.md`、设计上下文与项目约定后再发布。
