...
“已留存”是账面余额中的用途标记，不是第四种现金流。它回答“这些钱是否仍打算花掉”，并用于从总余额中得到可花余额。
...
Layer 1、Layer 2 与周期留存均已批准并实现：现金流与日常花法已分离；处理方式、确认状态、恢复分摊和留存事件写入 IndexedDB，进入加密备份 payload v4，并通过同步协议 v6 与 D1 持久化。分析结果本身仍只在客户端派生，不写入备份或云端。服务端仅在账本仍可无损表示时兼容 v1–v5；存在旧协议无法表达的语义时返回 `upgrade_required`。
...
- 分析结果本身只在本机派生；处理方式、确认元数据、恢复分摊和留存事件进入备份 payload v4 与同步协议 v6。
...
| 收入预期 | 下一次预计到账日的最低收入与预计收入 | 本次预计到账日、最低金额、预计金额 | 只用于下周期场景比较，不提前进入余额；本次日期可延期且不改变长期默认发薪日 |
| 留存目标 | 用户希望本周期新增保留、不再用于日常消费的金额 | 长期默认值、当前周期覆盖值 | 不代表真实银行账户，不改变总余额；本期缺口不自动结转 |
| 留存事件 | 对已留存用途的事实变更 | 初始留存、追加、取用、周期结算 | 独立于收支；直接用于支出时可与对应支出账目关联 |
...
| 总余额 / 已留存 / 可花余额 | 周期底线、存款账户 | 分开表达真实资金、用途标记和当前可支配金额 |
| 每周期留存目标 | 月底必须剩下 | 目标是本周期新增留存，不是绝对余额下限 |
...
- 留存、取用和周期结算不参与余额；它们只改变总余额中“已留存”的用途标记。
...
本周期净增长 = 当前已留存 - 周期开始时已留存
尚需留存 = max(本周期目标 - 本周期净增长, 0)
原始可花金额 = 当前总余额 - 当前已留存 - 尚需留存
留存差额 = 预计周期末总余额 - 当前已留存 - 尚需留存
每日可花 = max(原始可花金额, 0) ÷ 保守估算天数
```
...
- 尚未到账的收入预期不进入当前周期余额和每日可花。
- 取用旧留存会降低当前已留存，因此增加本周期尚需留存；同步冲突造成留存为负时不静默截断，而是进入待校正状态。
...
最低收入差额 = 最低收入 - 参考支出 - 下周期默认留存目标
预计收入差额 = 预计收入 - 参考支出 - 下周期默认留存目标
```

- 两个场景回答收入能否覆盖当前日常花法，不回答收入发生概率。
- 历史一次性收入和普通收入都不会自动成为未来收入预期。
- 两个场景都要分别展示收入、参考支出和默认留存目标，不把当前余额重复扣入下周期收入场景。
- 当前周期未完成的留存目标不自动结转到下周期。

### 5.6 留存事件与周期结算

- 初始留存、追加留存和取用金额必须为正；周期结算允许 0，明确表达“本周期未新增留存”。
- 同一周期只有一个有效结算事件，更正时更新该事件，不重复创建。
- 确认实际收入时，周期结算、实际收入账目、收入预期清除和下一周期初始化在同一 IndexedDB 事务中完成。
- 直接用留存支付时，同时创建真实支出和关联取用事件；支出编辑、软删除、8 秒撤销与最终清理必须联动。
- 留存事件不混入最近账目。分析页可以展示本周期进度、最近 6 个周期目标与净增长，以及可展开的留存明细。
...
| 浮动收入 | 实际到账后增加 | 不降低支出日均；未来只使用双收入预期 | 适合做情景比较，不是收入预测 |
...
- 不生成“包含异常支出 × 排除异常支出 × 最低收入 × 预计收入”的四场景矩阵。
...
| 完整工资周期支出 | 各周期全部外部实际流出 | 不冲减历史实际流出 | 计入，并在数据表列出非日常金额 |
...
- 留存事件使用独立表；与支出关联的取用事件必须和支出编辑、删除、撤销及 outbox 在同一事务中处理。
...
- 新备份恢复后必须保留处理方式、确认状态、恢复分摊、留存目标覆盖与留存事件。
- 恢复预检必须验证分摊引用、留存关联、方向、处理方式、累计金额和周期结算唯一性；任一失败不得修改现有数据。
- 预览应显示账目数，不需要暴露分析派生结果。

### 9.3 云同步

- 同步协议 v6 表达账目处理方式、确认元数据、恢复分摊、留存目标覆盖和独立留存事件，并继续接收可无损表示的 v1–v5 账本；旧客户端缺少新字段时不得静默清除云端已有语义。
- 一旦账号中存在账户转账、一次性支出、可报销支出、退款/报销、待确认状态、恢复分摊、非零留存目标或有效留存事件，无法表达相应语义的旧客户端不得继续执行云端拉取、推送或给出“同步完成”。服务端应返回明确的 `upgrade_required`，旧客户端保留本机数据并提示更新应用。
- 旧客户端离线期间产生的 outbox 必须在升级后由新客户端迁移再发送，不能由旧协议覆盖新语义；升级失败时本机账目不得丢失。
- 只有全部账目仍为默认普通收支、不存在恢复分摊且没有 v6 留存语义时，服务端才可以向相应旧协议投影现有账本。
- 新客户端读取旧账目时使用保守默认，不伪造退款或转账关系。
- 关联的两笔账目及关联金额需要作为可冲突、可恢复的关系处理；不能依赖客户端时间戳静默覆盖。
- D1 需要保存处理方式、确认状态、恢复分摊和留存事件，并校验同一用户代次、关联金额、周期日期与单周期结算唯一性。
...
| 处理方式、恢复分摊与留存事件持久化并同步 | 仅临时保存在界面或单设备 | 轻量确认和留存用途若不持久化就无法在刷新、备份和多端后生效 | 已批准并以备份 v4 / 同步 v6 实现 | 云同步范围被取消 |
...
4. 已升级加密备份 payload v4、同步协议 v6 和 D1 migration 0008，并实现旧客户端升级闸门。
5. 已使用代表性合成账本校准异常触发边界并补齐组件、数据库和浏览器回归测试。
6. 发布必须按 D1 migration 0008、兼容 v1–v6 的 Worker/Functions、前端的顺序进行。
