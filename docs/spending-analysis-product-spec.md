...
- 周期账单计入个人支出和实际现金流，但不因本次出现而外推为未来每天都会发生。
- 已到账的一次性收入影响实际余额，但不会自动成为后续周期的收入预期。
- 待报销支出结束前不计个人支出；退款或报销只有在用户确认关联后，才调整对应支出的分析口径，且到账不计个人收入。
- 快速记账仍然先保存；仅在交易明显影响分析时，保存后进行一次非阻塞的轻量确认。
- 固定账单预测继续不在当前范围内，因此结论必须明确限定为“按近期已记录花法估算”。

### 实施状态

现金流、个人收支与日常花法已经分离；处理方式、确认状态、恢复分摊、存钱事件和余额调整写入 IndexedDB v8，进入加密备份 payload v7，并通过同步协议 v9 与 D1 持久化。每期需存与其他分析结果仍只在客户端派生，不写入备份或云端。服务端仅在账本仍可无损表示时兼容 v1–v8；存在旧协议无法表达的余额调整或周期账单时返回 `upgrade_required`。
...
- 普通支出的净自付额进入日常花法；周期账单和一次性支出计入个人支出但不外推；待报销支出结束前不计个人支出，恢复分摊按用户确认金额追溯调整原始支出。
- 收入不会启动支出观察窗口，最近最多 30 个已完成自然日按线性比例外推，14 天以下不输出预测结论。
- 异常账目先持久化再提示；同一修订不重复主动弹出，待确认入口可恢复。
- 分析结果本身只在本机派生；处理方式、确认元数据、恢复分摊、存钱事件和余额调整进入备份 payload v7 与同步协议 v9。
...
| 处理方式 | 这笔账在分析中代表什么 | 普通收支、周期账单、一次性支出、待报销支出、退款/报销、账户转账 | 属于账目的分析属性，不是完整消费分类 |
...
| 周期账单 | 是，减少 | 是，计为流出 | 否 | 属于个人支出，但本次金额不用于外推日常花法 |
| 一次性支出 | 是，减少 | 是，计为流出 | 否 | 已经发生，但不假设未来重复 |
| 待报销支出 | 是，减少 | 是，计为流出 | 否 | 结束报销前个人支出为 0；部分报销后由用户决定剩余自付额的口径 |
| 普通收入 | 是，增加 | 是，计为流入 | 不适用 | 不自动沿用到未来周期 |
| 退款/报销 | 是，增加 | 是，单独标明恢复流入 | 不适用 | 不计个人收入；有效分摊按金额追溯抵减原支出 |
| 账户转账 | 否 | 否 | 否 | 用户明确确认资金只在自己的账户之间转移；产品不凭日期、金额或备注自动判定 |

如果退款或报销无法关联到原始账目，它仍作为恢复流入影响余额和现金流，但不计个人收入，也不得自动修改任意历史支出的分析口径。一笔恢复流入可以分摊给多笔支出，一笔支出也可以接受多次恢复；每笔分摊必须为正，累计不得超过恢复流入金额或原始支出尚未恢复的金额。待报销支出在结束前个人支出为 0；部分报销后用户通过“结束报销”把剩余自付额改为普通支出、周期账单或一次性支出，已有分摊继续保留。
...
- 软删除账目不参与余额。
- 普通收支、周期账单、一次性支出、待报销支出以及退款/报销都参与余额。
...
实际流出 = 普通支出 + 周期账单 + 一次性支出 + 待报销支出
现金流净额 = 实际流入 - 实际流出
```

- 经用户明确确认的账户转账不进入三个值；转账候选在确认前先按普通外部流入或流出处理，并保持待确认提示。
- 收入和支出必须分别可见，不能只展示净额。
- 退款/报销可在明细中单独标明，属于实际流入但不属于个人收入。
- 个人支出按原支出月份归属：普通、周期和一次性支出使用扣除有效分摊后的净自付额；待报销支出在结束前为 0。
- 部分退款或报销按实际关联金额计入恢复流入；结束报销后，原始支出的剩余自付金额按用户选择的最终处理方式统计。
- 现金流用于解释过去，不直接替代支出预测。
...
- 周期账单、一次性支出和待报销支出本来就不进入日常花法，恢复分摊只用于解释其资金恢复，不会产生负支出。
...
- 每月发薪日是长期默认值；新建下次收入默认取严格晚于今天的首个常规发薪日，临时改期不修改长期默认值。本次延期最多到下一常规发薪日前一天并保留原预期 ID；跨过该边界必须结束本次并以新 ID 建立下一笔。逾期确认按真实到账日写入账目。
...
- 支出确认线为 `max(¥200, ceil(日常净支出总额 × 3 ÷ 完整日数))`；满 14 个完整日后才使用动态值，数据不足时只使用 ¥200，任一阈值取等号即触发。
- 即使未达到大额阈值，若改变该支出的处理方式会使“到下次”够花/有缺口结论翻转，也触发确认。
- 普通收入达到 ¥500 时触发确认；正金额与此前 60 天内某笔支出的绝对金额相差不超过 ¥1 时，提示可能为退款/报销，但不自动建立恢复分摊。

建议的高信号触发条件：

1. 纳入或排除该支出会使“够用/有缺口”结论发生变化。
2. 单笔支出达到当前日常完整日日均的三倍，且不低于 ¥200。
...
- 确认层突出三个异常选项：
  - `周期账单`：计入个人支出和现金流，不进入日常预测。
  - `仅这一次`：计入个人支出和现金流，不进入日常预测。
  - `之后报销`：先影响余额和现金流，结束报销前个人支出为 0。
- 普通支出不作为第四张异常选项，使用 `按日常算`退出确认并计入日常花法。
- 账户转账保留在账目编辑入口，不与三个异常选项并列。
- 次要动作：`稍后`；账目保留在待处理队列。

### 6.4 收入确认内容

- 仅在金额达到 ¥500 或可能匹配退款/报销时出现。
- 标题：`确认资金来源`
- 说明：`账目已经保存。这个选择只影响余额解释和统计口径。`
- 选项：
  - `收入`：增加余额和实际流入，不自动用于未来收入预期。
  - `退款或报销`：增加余额，并进入关联原始支出的流程。
- 账户转账继续在编辑入口提供，不与收入确认选项并列。
...
- 待报销支出在记录行显示待报金额；不因长期等待而反复进入待处理队列。
- 全额分摊后显示“已报销”。部分报销可执行“结束报销”，把剩余自付额原子改为普通支出、周期账单或一次性支出；已有分摊保留。
- 退款、原支出或分摊被编辑、删除或撤销时，待报金额、个人收支和日常预测立即联动重算。
...
- 可花余额继续是首页最重要的确定性数字，总余额与已存金额作为紧凑摘要。
...
- “本月收入 / 本月支出”使用个人收支口径；真实流入与流出放入可展开现金流明细。
- “日常花法”按日常、周期、一次、待报和待确认拆分，展示数据覆盖天数。
- “本期流出”实线包含已发生的全部外部流出；预测虚线只使用日常花法基线。
- “周期流出”展示完整到账周期的真实外部流出；“每日支出”只展示日常净支出。
- 不生成“包含异常支出 × 排除异常支出 × 多档收入”的场景矩阵。
- 数据表与屏幕阅读器说明必须使用与可见界面一致的口径。

关键指标与图表使用以下固定口径：

| 指标或序列 | 使用金额 | 退款/报销处理 | 周期、一次性与待报销支出 |
|---|---|---|---|
| 当前余额 | 截至当前时刻的全部外部净流入 | 到账时增加余额 | 已发生时减少余额 |
| 本月收入 | 普通外部收入 | 不计入收入，单列为恢复流入 | 不适用 |
| 本月支出 | 原支出月份的个人净自付额 | 分摊追溯冲减原支出；待报销结束前为 0 | 周期和一次性计入；待报销结束前不计 |
| 本月实际流入/流出 | 全部外部资金进出 | 退款/报销计入流入 | 全部计入流出并保持可见 |
| 本月现金流净额 | 全部外部流入减全部外部流出 | 计入外部流入 | 计入外部流出 |
| 日常支出日均 | 普通支出净分析金额 | 按分摊金额归回原支出日期抵减 | 不计入 |
| 本期流出 | 全部外部实际流出累计 | 不冲减实际累计线 | 已发生部分保留，预测段不重复外推 |
| 本期预测流出 | 截至今天的实际外部流出加未来日常花法预测 | 只通过日常支出基线影响未来段 | 已发生部分保留，未来不重复外推 |
| 周期流出 | 各完整到账周期的全部外部实际流出 | 不冲减历史实际流出 | 全部计入真实流出 |
| 每日支出 | 按原支出日期聚合的日常净自付额 | 有效分摊追溯抵减 | 不计入 |

账户转账和尚未到达发生时刻的未来账目不进入上述任何金额序列。恢复分摊只改变日常花法基线，不改写已经发生的毛流入或毛流出历史。

### 8.3 账目记录

- 记录行使用“周期账单”“仅这一次”“待报 ¥X”“已报销”“自付 ¥X”等短标签，普通收支不增加标签噪声。
- 待报销支出提供“结束报销”，全额报销不要求额外关闭；账户转账只在编辑入口出现。
- 编辑记录时可以随时修改处理方式。
...
- `LedgerEntry` 的处理方式增加 `periodic_expense`；确认检测规则版本升级为 2。
- 恢复分摊应使用单一权威关系对象，保存恢复流入账目 ID、原始支出账目 ID 和分摊金额；不得在两条账目上分别维护可能不一致的双向副本。
- IndexedDB 升级到 v8，store 索引不变并作为新处理语义的旧标签页隔离；历史账目不自动改写处理方式。
- 历史检测规则 v1 或无版本账目不得因新 ¥200 阈值批量进入待处理；新建及金额、方向或时间发生实质变化的账目使用 v2。
...
- 同步协议 v9 表达周期账单、确认元数据、恢复分摊、独立存钱目标、单一预计收入、存钱事件和余额调整，并继续接收可无损表示的 v1–v8 账本；实际收入确认继续使用不进入账本或备份的服务端幂等凭据。
- D1 使用 `analysis_treatment` 保存 v9 权威处理方式，旧 `treatment` 作为兼容影子；周期账单仅在旧影子中投影为一次性支出。账号存在周期账单时，v1–v8 返回 `upgrade_required`，不得覆盖新语义或给出“同步完成”。
...
- D1 迁移 `0012_periodic_expense.sql` 保存权威处理方式并修复恢复分摊触发器，使带有效分摊的支出可在普通、周期、一次性和待报销之间合法转换。
...
- 周期账单减少余额、增加个人支出和真实流出，但不增加日常支出基线。
- 一次性支出减少当前余额和增加实际流出，但不增加日常支出基线。
- 待报销支出仍减少当前余额并增加真实流出，但结束报销前个人支出为 0。
- 全额报销到账后增加当前余额但不增加个人收入；关联成功后显示已报销。
- 部分退款或报销后显示剩余待报金额；结束报销后仅剩余自付额按所选日常、周期或一次性口径统计。
...
- 支出确认只突出周期账单、仅这一次和之后报销；按日常算可直接退出，账户转账仍可在编辑入口设置。
- 部分报销可结束为三种最终支出口径，保存失败时选择和分摊均不得丢失。
...
| 事项 | 当前判断 | 风险 | 后续决定 |
|---|---|---|---|
| 异常交易触发阈值 | 支出取 ¥200 与完整日日均 3 倍的较大值，不足 14 天只用 ¥200；收入 ¥500；退款 ±¥1/60 天 | 过低会打扰，过高会遗漏 | 规则版本变化或用户测试显示系统性误报时复审 |
...
| 处理方式、恢复分摊与存钱事件持久化并同步 | 仅临时保存在界面或单设备 | 轻量确认和存钱用途若不持久化就无法在刷新、备份和多端后生效 | 已批准并以备份 v7 / 同步 v9 实现 | 云同步范围被取消 |
| 非默认语义要求旧客户端升级 | 继续向旧客户端投影所有账目 | 旧客户端无法计算正确余额和预测，兼容投影会造成假一致 | 已批准并实现 `upgrade_required` | 旧协议获得等价语义支持 |

## 14. 实施记录

1. 已修正统计窗口，收入记录不能启动支出数据覆盖。
2. 已增加周期账单、待报销闭环、确认状态、恢复分摊和保守迁移，并分开计算现金流、个人收支与日常花法。
3. 已实现保存后轻量确认、待确认恢复入口、结束报销与分析页口径拆分。
4. 已升级 IndexedDB v8、加密备份 payload v7、同步协议 v9 和 D1 migration `0012_periodic_expense.sql`，并实现旧客户端升级闸门。
5. 已按 ¥200 / 完整日日均 3 倍的新规则校准异常触发边界并补齐组件、数据库和浏览器回归测试。
6. 发布必须按 D1 `0012`、兼容 v1–v9 的 Worker/Functions、前端的顺序进行。
