...
Layer 1 与 Layer 2 均已批准并实现：现金流与日常花法已分离；处理方式、确认状态和恢复分摊写入 IndexedDB，进入加密备份 payload v3，并通过同步协议 v5 与 D1 持久化。分析结果本身仍只在客户端派生，不写入备份或云端。服务端仅在账本仍可无损表示时兼容 v1–v4；存在非默认语义时返回 `upgrade_required`。
...
- 分析结果本身只在本机派生；处理方式、确认元数据和恢复分摊进入备份 payload v3 与同步协议 v5。
...
- 同步协议 v5 表达账目处理方式、确认元数据和独立的恢复分摊实体，并继续接收 v1–v4 的默认语义账目；旧客户端缺少新字段时不得静默清除云端已有处理方式。
...
| 处理方式与恢复分摊持久化并同步 | 仅临时保存在界面或单设备 | 轻量确认若不持久化就无法在刷新、备份和多端后生效 | 已批准并以备份 v3 / 同步 v5 实现 | 云同步范围被取消 |
...
4. 已升级加密备份 payload v3、同步协议 v5 和 D1 migration 0007，并实现旧客户端升级闸门。
5. 已使用代表性合成账本校准异常触发边界并补齐组件、数据库和浏览器回归测试。
6. 发布必须按 D1 migration 0007、Worker/Functions、前端的顺序进行。
