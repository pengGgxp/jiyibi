...
test("核心记账布局在当前视口可用且无障碍", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("−¥25.80", { exact: true })).toBeVisible();
  ...
  if (testInfo.project.name === "mobile-chrome") {
    ...
    expect(layout.summaryTop).toBeGreaterThan(layout.composerTop);
  } else {
  ...
  const tooSmallTargets = await page.locator("button:visible, summary:visible").evaluateAll((targets) =>
    targets
      .map((target) => {
