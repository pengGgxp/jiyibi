...
- 保存后对明显影响估算的大额交易进行轻量确认，可标记日常、一次性、可报销、退款/报销或账户转账；也可稍后从待确认入口恢复
...
- 存钱目标由截止日期和目标总额组成，不随发薪周期重置，也不会预先扣减可花余额。分析页可按剩余目标和截止日前的常规发薪次数给出“每期建议”，该建议不保存、不自动存钱。
...
- 账目、处理方式、确认状态、恢复分摊、存钱事件、初始余额、发薪日、存钱目标、下一次收入预期和截图先写入当前浏览器的 IndexedDB。未开启同步时，不会上传账本数据。
...
- `/api/sync` 的账本协议当前为 v7，并继续接受可无损表示的 v1–v6 账本。v7 使用独立的发薪日、单一预计收入和累计存钱目标；存钱事件仍按 v6 结构同步。实际收入确认通过 D1 内部凭据与正金额账目原子提交，0 元确认不创建账目，但同样可跨设备去重。旧协议 mutation 不得清除云端新字段；账号一旦存在旧协议无法表达的新存钱目标或迁移提示，服务端返回 `409 upgrade_required`，旧客户端必须升级且不能误报同步完成。
...
首次部署前以及新增迁移后运行。当前版本要求依次应用 `0009_savings_goal.sql` 和 `0010_savings_goal_compatibility_fix.sql`，再发布支持协议 v1–v7 的 Worker 与前端：
