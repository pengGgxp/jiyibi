...
- 当前余额、初始余额、分析页本月收支概览，以及可选的发薪日和周期末余额底线
...
分析页的预测只根据本机已有的有效账目实时计算，不会写入账本，也不会上传新的统计数据。页面使用“下个工资周期”而不是自然月：周期从最近一次发薪日开始，到下一次发薪日前一天结束；发薪日填写 29、30 或 31 日时，短月使用该月最后一天。

- 最近最多 30 个已完成自然日构成消费速度基线，期间没有支出的日期按 0 计入。
- 少于 14 天记录时只展示实际金额，不给出“够/不够”的预测；14–29 天标为“初步估算”，满 30 天才标为“按近 30 天估算”。
- 只纳入有效支出记录；实际收入影响余额，但不会降低消费速度，收入预期也不会提前补充余额。
- 当前周期会估算预计周期末余额，并与设置的周期末余额底线比较；下个周期分别比较最低收入、预计收入与按当前花法推导的参考支出。
- 预计结果不是保证，不包含固定支出、分类预算或异常支出判断。今天按完整消费日计入，属于偏保守的估算。

## 数据与隐私

- 金额以人民币“分”的整数形式保存，避免浮点误差。
- 账目、初始余额、发薪周期、下一次收入预期和截图先写入当前浏览器的 IndexedDB。未开启同步时，不会上传账本数据。
...
- `/api/sync` 的账本协议当前为 v4，并继续接受 v1/v2/v3 请求。v1 看不到可选规划字段；v2 只支持旧版自然月目标；v3 只识别带单个工资值的旧工资周期；v4 使用 `payCycle + incomeForecast`。旧协议或缺少字段的 mutation 会保留云端新字段，v3 写入只能更新兼容影子工资，不能覆盖或清除 v4 的双收入预期。旧云端工资会由首个 v4 客户端按本地日期认领为一次性收入预期并回写；预计收入为 0 时无法无损投影到 v3，因此旧客户端暂不显示该工资周期。
...
首次部署前以及新增迁移后运行。双收入场景要求先应用 `0006_income_forecast.sql`，再发布 Worker 与前端：
