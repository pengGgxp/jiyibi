...
- 当前余额、初始余额、分析页本月实际现金流，以及可选的发薪日和周期末余额底线
- 保存后对明显影响估算的大额交易进行轻量确认，可标记日常、一次性、可报销、退款/报销或账户转账；也可稍后从待确认入口恢复
- 为下一次发薪日单独填写最低收入与预计收入；预期不会提前计入余额，也不会自动沿用到后续周期
- 发薪日到期后确认实际收入；正金额才生成收入账目，确认 0 元不会生成账目
...
分析页的预测只根据本机已有的有效账目实时计算，不会写入账本，也不会上传新的统计数据。产品同时维护三种口径：账面余额表示全部个人可用资金，实际现金流分别展示外部流入和流出，日常花法只用具有未来代表性的普通支出来估算。页面使用“下个工资周期”而不是自然月：周期从最近一次发薪日开始，到下一次发薪日前一天结束；发薪日填写 29、30 或 31 日时，短月使用该月最后一天。
...
- 当前周期会估算预计周期末余额，并与设置的周期末余额底线比较；下个周期分别比较最低收入、预计收入与按当前花法推导的参考支出。
- 预计结果不是保证，不包含固定支出、分类预算或记账完整度判断。今天按完整消费日计入，属于偏保守的估算。

## 数据与隐私

- 金额以人民币“分”的整数形式保存，避免浮点误差。
- 账目、处理方式、确认状态、恢复分摊、初始余额、发薪周期、下一次收入预期和截图先写入当前浏览器的 IndexedDB。未开启同步时，不会上传账本数据。
...
- `/api/sync` 的账本协议当前为 v5，并继续接受可无损表示的 v1–v4 默认语义账本。v1 看不到可选规划字段；v2 只支持旧版自然月目标；v3 只识别带单个工资值的旧工资周期；v4 使用 `payCycle + incomeForecast`；v5 额外同步处理方式、确认元数据和恢复分摊。旧协议 mutation 不得清除云端新字段；账号一旦存在旧协议无法表达的非默认语义，服务端返回 `409 upgrade_required`，旧客户端必须升级且不能误报同步完成。
...
首次部署前以及新增迁移后运行。交易处理语义要求先应用 `0007_entry_treatment_recovery_allocations.sql`，再发布 Worker 与前端：
