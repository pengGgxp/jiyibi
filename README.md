...
- 保存后对明显影响估算的大额交易进行轻量确认，可标记日常、一次性、可报销、退款/报销或账户转账；到账、异常交易、存款穿透与退款关联统一进入“待处理”
- 退款或报销可关联一笔或多笔原支出，处理方式与分摊原子保存；编辑、删除和 8 秒撤销会联动分摊
...
- 分析页提供当前周期累计支出、最近 6 个完整到账周期和近 30 天每日支出的可视化图表，并可展开数据表
...
- 普通支出进入日常花法；一次性、可报销和账户转账不用于外推。账户转账也不进入账面余额或实际现金流；退款/报销可通过恢复分摊抵减原始支出的分析金额。
- 未确认支出先按普通支出保守计入。大额或可能改变结论的支出，以及接近近期支出金额的收入，会在保存成功后提示处理方式；同一修订只主动提示一次。
...
- `/api/sync` 的账本协议当前为 v8，并继续接受可无损表示的 v1–v7 账本。v8 新增余额调整实体和不可清除的初始余额锁；实际收入确认仍通过 D1 内部凭据原子提交。没有有效余额调整时，旧客户端仍可同步，但不得修改已锁定起点；存在有效余额调整后，v1–v7 返回 `409 upgrade_required`，旧客户端必须升级且不能误报同步完成。
...
首次部署前以及新增迁移后运行。当前版本要求在既有 `0010` 后应用 `0011_balance_adjustments.sql`，再发布支持协议 v1–v8 的 Worker 与前端：
