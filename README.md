...
- 当前余额、初始余额、分析页本月收支概览，以及可选的发薪日、每月工资和周期末余额底线
- 工资周期内展示实际支出、周期末余额预测和每日可花金额；不预测固定支出，也不会自动生成工资收入
- “分析”页面：按最近 30 天已记录支出估算到发薪日是否够用，以及下个工资周期工资是否覆盖当前花法
...
- 只纳入有效支出记录；收入影响实际余额，但不会降低消费速度或自动补充工资预算。
- 当前周期会估算预计周期末余额，并与设置的周期末余额底线比较；下个周期单独比较月工资与预计支出。
