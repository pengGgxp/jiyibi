...
- 可花余额、总余额、已有存款、分析页本月实际现金流，以及可选的发薪日和单一存钱目标
- 空账本优先快速记账；第一笔事实后渐进显示资金判断、收入预期和存钱目标
- 独立记录已有存款、存入与取用；存钱不算消费，也不改变总余额
- 保存后对明显影响估算的大额交易进行轻量确认，突出“周期账单 / 仅这一次 / 之后报销”；普通日常支出可直接退出确认，账户转账保留在编辑入口
- 退款或报销可关联一笔或多笔原支出；支持部分、多次和一笔到账分摊多笔支出，编辑、删除和 8 秒撤销会联动分摊
- 待报销支出结束前不算个人支出；部分报销后可结束报销，将剩余自付额按日常、周期账单或仅这一次统计
- 第一笔账本事实后锁定初始余额；现实金额不一致时使用“余额校准”，起点填错时使用“更正起点”，两者都保留审计记录
- 为下一次预计到账日填写一个预计收入；默认使用严格晚于今天的首个常规发薪日，也可单次改期。延期本次不会改变每月默认日，也不会与下一笔收入共用同一预期
- 预计到账日到期后确认实际收入；正金额才生成收入账目，确认 0 元不会生成账目，也不会自动创建下一次预期
- “分析”页面：按最近 30 天已记录支出估算到下次到账是否够用，并比较预计收入能否覆盖随后一个到账周期的当前花法
- 分析页提供本期流出、最近 6 个完整周期流出和近 30 天日常支出的可视化图表，并可展开数据表
- 记录编辑、软删除与 8 秒撤销；余额调整也提供 8 秒软作废且长期保留审计行
- 密码加密的单文件备份与整体恢复
...
分析页的预测只根据本机已有的有效账目实时计算，不会写入账本，也不会上传新的统计数据。产品分开维护总余额、存钱用途、实际现金流和日常花法：总余额表示全部个人可用资金；已存金额仍属于总余额，但不计入可花余额；实际现金流分别展示外部流入和流出；日常花法只用具有未来代表性的普通支出来估算。到账周期通常按每月默认发薪日划分；若本次预计到账日延期，当前周期延长到实际目标日前一天，随后周期从该目标日开始并在下一常规发薪日前结束。发薪日填写 29、30 或 31 日时，短月使用该月最后一天。

- 最近最多 30 个已完成自然日构成日常花法基线，期间没有支出的日期按 0 计入。
- 少于 14 天记录时只展示实际金额，不给出“够/不够”的预测；14–29 天标为“初步估算”，满 30 天才标为“按近 30 天估算”。
- 普通支出的净自付额进入日常花法；周期账单和一次性支出计入个人支出但不外推；待报销支出结束前不计个人支出，账户转账不进入账面余额或实际现金流。
- 退款/报销到账属于真实流入但不算个人收入；有效分摊追溯冲减原支出，部分报销结束后只有剩余自付额进入用户选择的支出口径。
- 未确认支出先按普通支出保守计入。支出确认线为 `max(¥200，完整日日均支出 × 3)`；不足 14 个完整日时只使用 ¥200。可能改变结论的支出，以及达到 ¥500 或接近近期支出金额的收入，也会在保存成功后提示；同一修订只主动提示一次。
- 实际收入影响余额但不会降低消费速度，收入预期也不会提前补充余额。
- “到下次”使用 `总余额 - 已存金额 - 到账前预计日常支出`；“下次收入”使用 `预计收入 - 随后周期预计日常支出`。存钱目标不参与这两项判断。
- 存钱目标由截止日期和目标总额组成，不随发薪周期重置，也不会预先扣减可花余额。分析页可按剩余目标和截止日前的常规发薪次数给出“每期需存”，它只是目标均摊，不保存、不自动存钱，也不代表可行性建议。
- 同步合并造成已存金额为负时，应用会要求校正而不是静默截断。历史周期结算继续作为旧版存入计入已存金额，但新版本不再创建周期结算。
- 预计结果不是保证，不包含固定支出、分类预算或记账完整度判断。今天按完整消费日计入，属于偏保守的估算。

## 数据与隐私

- 金额以人民币“分”的整数形式保存，避免浮点误差。
- 账目、处理方式、确认状态、恢复分摊、存钱事件、初始余额锁、余额调整、发薪日、存钱目标、下一次收入预期和截图先写入当前浏览器的 IndexedDB。未开启同步时，不会上传账本数据。
- 用户登录并确认开启同步后，账目与同步元数据的云副本写入 D1，压缩后的 JPEG 截图云副本写入绑定名为 `ATTACHMENTS` 的 Workers KV；本机数据仍保留。
- 云同步通过 HTTPS 传输并受应用会话保护，但云副本不是端到端加密；Cloudflare 运行环境可处理服务端明文数据。只有密码加密的 `.jiyibi` 文件使用端到端的用户密码保护。
...
- `/api/sync` 的账本协议当前为 v9，并继续接受可无损表示的 v1–v8 账本。v9 新增周期账单处理方式，v8 的余额调整实体和不可清除初始余额锁继续保留；实际收入确认仍通过 D1 内部凭据原子提交。旧客户端无法表达有效余额调整或周期账单时返回 `409 upgrade_required`，不得覆盖新语义或误报同步完成。
...
首次部署前以及新增迁移后运行。当前版本要求在既有 `0011_balance_adjustments.sql` 后应用 `0012_periodic_expense.sql`，再发布支持协议 v1–v9 的 Worker 与前端：
