...
- 可花余额、总余额、初始余额、分析页本月实际现金流，以及可选的发薪日和每周期留存目标
- 独立记录初始留存、追加留存、取用与周期结算；留存不算消费，也不改变总余额
- 保存后对明显影响估算的大额交易进行轻量确认，可标记日常、一次性、可报销、退款/报销或账户转账；也可稍后从待确认入口恢复
- 为下一次预计到账日单独填写最低收入与预计收入；公司临时延期时可只调整本次日期，不改变每月默认发薪日
- 预计到账日到期后确认实际收入；正金额才生成收入账目，确认 0 元不会生成账目，确认后新建预期默认进入下一周期
- “分析”页面：按最近 30 天已记录支出估算到发薪日是否够用，并分别比较最低收入、预计收入能否覆盖下个周期的当前花法
- 分析页提供当前周期累计支出、最近 6 个完整工资周期和近 30 天每日支出的可视化图表，并可展开数据表
- 记录编辑、软删除与 8 秒撤销
- 密码加密的单文件备份与整体恢复
...
分析页的预测只根据本机已有的有效账目实时计算，不会写入账本，也不会上传新的统计数据。产品分开维护总余额、留存用途、实际现金流和日常花法：总余额表示全部个人可用资金；已留存仍属于总余额，但不计入可花余额；实际现金流分别展示外部流入和流出；日常花法只用具有未来代表性的普通支出来估算。页面使用“下个工资周期”而不是自然月：通常按每月默认发薪日划分；若本次预计到账日延期，当前周期延长到实际目标日前一天，下个周期从该目标日开始并在下一常规发薪日前结束。发薪日填写 29、30 或 31 日时，短月使用该月最后一天。
...
- 当前周期会估算预计周期末总余额，并与“已留存 + 尚需留存”比较；每日可花只使用扣除这两项后的金额。下个周期分别比较最低收入、预计收入能否覆盖参考支出和默认留存目标。
- 本周期尚未完成的留存目标不会自动结转；取用旧留存会提高本周期尚需留存金额。同步合并造成留存为负或重复结算时，应用会要求校正而不是静默截断。
- 预计结果不是保证，不包含固定支出、分类预算或记账完整度判断。今天按完整消费日计入，属于偏保守的估算。

## 数据与隐私

- 金额以人民币“分”的整数形式保存，避免浮点误差。
- 账目、处理方式、确认状态、恢复分摊、留存事件、初始余额、发薪周期、留存目标、下一次收入预期和截图先写入当前浏览器的 IndexedDB。未开启同步时，不会上传账本数据。
...
- `/api/sync` 的账本协议当前为 v6，并继续接受可无损表示的 v1–v5 账本。v1 看不到可选规划字段；v2 只支持旧版自然月目标；v3 只识别带单个工资值的旧工资周期；v4 使用 `payCycle + incomeForecast`；v5 额外同步处理方式、确认元数据和恢复分摊；v6 增加每周期留存目标、单期覆盖和独立留存事件。旧协议 mutation 不得清除云端新字段；账号一旦存在旧协议无法表达的非默认语义，服务端返回 `409 upgrade_required`，旧客户端必须升级且不能误报同步完成。
...
首次部署前以及新增迁移后运行。周期留存要求先应用 `0008_savings_events.sql`，再发布支持协议 v1–v6 的 Worker 与前端：
